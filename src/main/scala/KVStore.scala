...
case class Put(key: BigInt, value: ListBuffer[ActorRef]) extends KVStoreAPI
case class Get(key: BigInt) extends KVStoreAPI
case class Keys(key:BigInt) extends KVStoreAPI
...
class KVStore extends Actor {
  ...
  private val groupStore = new scala.collection.mutable.HashMap[BigInt, Any]

  override def receive = {
    case Put(key, cell) =>
      sender ! store.put(key,cell)
      //sender ! groupStore.put(key,cell)
    case Get(key) =>
      sender ! store.get(key)
      //sender ! groupStore.get(key)
  }
}
