...
class AnyMap extends scala.collection.mutable.HashMap[BigInt, ListBuffer[ActorRef]]
...
class KVClient (stores: Seq[ActorRef]) {
  ...
  /** Cached read */
  def read(key: BigInt): Option[ListBuffer[ActorRef]] = {
  ...
  def write(key: BigInt, value: ListBuffer[ActorRef], dirtyset: AnyMap) = {
    cache.put(key, value)
  ...
  def directRead(key: BigInt): Option[ListBuffer[ActorRef]] = {
    val future = ask(route(key), Get(key)).mapTo[Option[ListBuffer[ActorRef]]]
    Await.result(future, timeout.duration)
  }

  /*
  /** Direct read, bypass the cache: always a synchronous read from the store, leaving the cache unchanged. */
  def directKeys(key: BigInt): Option[ListBuffer[BigInt]] = {
    val future = ask(route(key), Keys(key).mapTo[Option[ListBuffer[BigInt]]]
    Await.result(future, timeout.duration)
  }
*/

  /** Direct write, bypass the cache: always a synchronous write to the store, leaving the cache unchanged. */
  def directWrite(key: BigInt, value: ListBuffer[ActorRef]) = {
    val future = ask(route(key), Put(key,value)).mapTo[Option[ListBuffer[ActorRef]]]
    Await.result(future, timeout.duration)
  }
